...
public class ValidatorDonorProfile implements IValidator<DonorProfile> {
    ...
    public void validate(DonorProfile entity) throws ValidationException {
        ...
        if (entity.getCNP() == null || entity.getCNP().length() != 13 || patternCNPAndPhone.matcher(entity.getCNP()).find())
            errors += "Invalid CNP\n";
        if (entity.getEmail() == null || entity.getEmail().equals("") || !patternEmail.matcher(entity.getEmail()).find())
        ...
        if (entity.getBirthDate() == null || entity.getBirthDate().toString().equals("") || entity.getBirthDate().after(Date.valueOf(LocalDate.now())))
            errors += "Invalid BirthDate\n";
        if (entity.getFirstName() == null || entity.getFirstName().equals("")) errors += "Invalid FirstName\n";
        if (entity.getLastName() == null || entity.getLastName().equals("")) errors += "Invalid LastName\n";
        if (entity.getNationality() == null || entity.getNationality().equals("") || patternNationality.matcher(entity.getNationality()).find())
            errors += "Invalid Nationality\n";
        if (entity.getHeight() <= 0) errors += "Invalid Height\n";
        if (entity.getWeight() <= 0) errors += "Invalid Weight\n";
        if (entity.getPhone() == null || entity.getPhone().equals("") || patternCNPAndPhone.matcher(entity.getPhone()).find())
